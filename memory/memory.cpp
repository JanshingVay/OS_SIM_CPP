...
MemoryManager::MemoryManager() : tlb(16) {
    for (int i = 0; i < TOTAL_PAGES; i++) {
        ...
        ref_bit[i] = false;
        std::memset(ram_frames[i], 0, PAGE_SIZE);
    }
...
bool MemoryManager::alloc_mem(int pid, int size) {
    ...
    // 预填合法逻辑页的页表项：初始 valid=false，真正访问时触发缺页并调入物理帧。
...
bool MemoryManager::evict_frame_to_disk(int frame) {
    ...
    std::memcpy(buffer, ram_frames[frame], PAGE_SIZE);
...
bool MemoryManager::translate(int pid, size_t logical_addr, int& out_physical_frame, size_t& out_offset_in_page) {
    ...
    size_t logical_page = logical_addr / PAGE_SIZE;
...
bool MemoryManager::share_mem(int pid1, int pid2, int num_pages) {
    ...
    for (int i = 0; i < num_pages; ++i) {
        PTE* pb = walk_page_table(pid2, i, false);
        if (pb) {
        ...
        PTE* pa = walk_page_table(pid1, i, false);
        if (pa) {
            ...
            if (pa->valid) {
                ...
                frame_mappings[f].push_back({ pid2, i });
...
bool MemoryManager::handle_page_fault(int pid, uint32_t logical_page) {
    ...
    if (pte->swap_block_id >= 0) {
        if (!get_disk_manager().read_data_block(pte->swap_block_id, reinterpret_cast<char*>(ram_frames[target_frame]))) {
        ...
        std::memset(ram_frames[target_frame], 0, PAGE_SIZE);
    }
