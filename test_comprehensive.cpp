...
void test_filesystem() {
    ...
    // 清理上一次异常中断可能遗留的测试目录，避免重复运行时 create 失败。
    iNode old_dir;
    if (fs.get_file_info("test_dir", old_dir) && old_dir.i_mode == 0) {
        fs.cd("test_dir");
        fs.set_file_permission("world.txt", false);
        fs.delete_file("world.txt");
        fs.cd("..");
        fs.delete_file("test_dir");
    }
    fs.pwd();
    ...
    check("偏移写入", "在偏移7处写入4字节", written_at == 4);
    std::string after_patch = fs.read_file("hello.txt");
    check("偏移写入", "保留原文件尾部", after_patch == "Hello, TestIM_CPP!");
