...
#define SCHED_FCFS 2
#define SCHED_HRRN 3
#define SCHED_MLFQ 4 // [新功能: MLFQ] 多级反馈队列

#define MAX_CORES 2 // [新功能: SMP] 模拟多核处理器的核心数量
#define MLFQ_LEVELS 3 // [新功能: MLFQ] 多级反馈队列数量
...
typedef struct PCB {
    ...
    // ================= [新增: 5项高级特性底层字段] =================
    int pgid = 0;                      // [Job Control] 进程组ID
    int maxChildrenLimit = 999;        // [ulimit] 最大允许子进程数
    std::deque<int> pendingSignals;    // [Signals] 待处理的软中断信号队列
    int queueLevel = 0;                // [MLFQ] 队列层级 (0:最高级/时间片小, 1:中, 2:最低级/时间片大)
    // ===============================================================
...
extern std::vector<PCB> readVector;            // 兼容旧接口的就绪队列快照 (Ready Queue)
extern std::vector<PCB> readQueues[MLFQ_LEVELS]; // [新功能: MLFQ] 分级就绪队列 0最高级
...
extern int cpuCores[MAX_CORES];                // [新功能: SMP] 多核运行数组
extern int currentRunningPID;                  // 兼容单核老接口 (始终等于 cpuCores[0])
...
int sendSignal(int pid, int signum);             // [Signals] 向 PCB 投递软中断信号
int setMaxChildrenLimit(int pid, int limit);     // [ulimit] 修改最大子进程数
int setProcessGroup(int pid, int pgid);          // [Job Control] 设置进程组
int killGroup(int pgid);                         // [Job Control] 终止整个进程组
