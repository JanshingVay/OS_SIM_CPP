...
int FileSystem::write_file(const std::string &filename, const std::string &data, int offset)
{
    ...
    int new_size = offset + static_cast<int>(data.size());
    int old_size = file_inode.i_size;

    // 约定：offset == 0 的 write 表示“完整覆盖并截断”；offset > 0 的 write_at 表示
    // 在指定偏移处就地修改，保留原文件尾部内容。这样既保留原有 write 的截断语义，
    // 又避免 write_at a.txt 2 YY 把 hello 错误截成 heYY。
    bool truncate_tail = (offset == 0 && new_size < old_size);
    if (truncate_tail)
    {
